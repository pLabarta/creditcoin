...
export function makeCollectCoinsCommand() {
  const cmd = new Command("collect-coins");
  cmd.description("Register Ethereum wallet with Creditcoin and collect CTC");
...
async function collectCoinsAction(options: OptionValues) {
  ...
  const collectCoinsVerified = await collectCoinsEvent
